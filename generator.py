...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x = x.to(self.device)  #input is on right device
        emb = self.emb(x)
    ...
    def init_hidden(self, batch_size):
        # 初始化 hidden 和 cell 状态时，将其放到指定的 device 上
    ...
    def sample(self, batch_size, seq_len, x=None):
        ...
        if x is None:
            flag = True
        if flag:
            x = Variable(torch.zeros((batch_size, 1)).long()).to(self.device)  # 将输入张量放置到正确的设备上
